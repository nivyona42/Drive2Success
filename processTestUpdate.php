...
<main>
    <?php
	   $host="localhost";
        ...
        $sql = "UPDATE tests SET idStudent='$idStudent', date='$date', startTime='$startTime',endTime='$endTime', type='$type', result='$result'  WHERE idStudent='$prevId' AND date='$prevDate'";
        echo $sql;

               if ($conn->query($sql) == FALSE) {
            echo "<p class='echoDesign'>לא ניתן לעדכן את פרטי המבחן , שגיאה: " . $conn->error . "</p>";
        } else {
            echo "<p class='echoDesign'>פרטי המבחן עודכנו בהצלחה!</p>";
        }
    ?>
     <div class="center">
