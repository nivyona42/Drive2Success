    ...
    <header class="container-fluid sticky-top" >
        <nav class="navbar shadow p-3 mb-5 bg-body rounded " dir="rtl">
            <div class="container-fluid">
                ...
                <span id="menuHeading" class="navbar-text ms-auto">&nbsp; פרטי כלי רכב </span>
    ...
    <main>
        ...
        if (mysqli_num_rows($result) > 0) {
        		while($row = mysqli_fetch_assoc($result)) {
        		    echo "<p class='echoDesign'>מספר רכב: ".$row["id"]."</p>";
        		    echo "<hr>";
        		    echo "<p class='echoDesign'>חברה: ".$row["company"]."</p>";
        		    echo "<hr>";
        		    echo "<p class='echoDesign'>מודל: ".$row["model"]."</p>";
        		    echo "<hr>";
        		    echo "<p class='echoDesign'>שנת ייצור: ".$row["year"]."</p>";
        		    echo "<hr>";
        		    echo "<p class='echoDesign'>צבע: ".$row["color"]."</p>";
        		    echo "<hr>";
        		    echo "<p class='echoDesign'>גיר: ".$row["gir"]."</p>";
        		    echo "<hr>";
        		    echo "<p class='echoDesign'>מחיר לשיעור: ".$row["price"]."</p>";
        		    echo "<hr>";
        		    echo "<p class='echoDesign'>תאריך פקיעת ביטוח: ".$row["insuranceDate"]."</p>";
        		    echo "<hr>";
        		    echo "<p class='echoDesign'>תאריך פקיעת טסט: ".$row["testDate"]."</p>";
        		    echo "<hr>";
        		}
        }
        ...
        <button class="delete" onclick="window.location.href = 'updateCar.php?id=<?php echo $id;?>'">ערוך</button>
        <div class="echoDesign" id="note" style="display:none;">
         האם אתה בטוח שברצונך למחוק את כלי רכב שמספרו <?php echo $id; ?>?
            <button class="yesNo" onclick="window.location.href = 'deleteCar.php?id=<?php echo $id;?>'">כן</button>
