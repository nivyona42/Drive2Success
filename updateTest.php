...
<main>
    <?php
        ...
        $sql = "SELECT * FROM tests WHERE idStudent='$id' AND date='$date'";
       ...
       if ($result->num_rows > 0) {
        ...
        <section class="container col-sm-5">
  			<form method="post" action="processTestUpdate.php?date=<?php echo urlencode($date); ?>&id=<?php echo $id; ?>" name="myForm" onsubmit="return validateForm()" novalidate>
				<h1 class="center"><b>עדכון פרטי מבחן</b></h1>
				...
				<div class="row">
					<div class="col-sm-6">
						<label for="type">סוג מבחן</label>
							<select id="type" class="form-group mb-3" name="type" required>
							    <option value="טסט פנימי"<?php if ($type == 'טסט פנימי') echo ' selected'; ?>>טסט פנימי</option>
                                <option value="טסט חיצוני"<?php if ($type == 'טסט חיצוני') echo ' selected'; ?>>טסט חיצוני</option>
							</select>
					</div>
                ...
                <div class="row">
					<div class="col-sm-6">
						<label for="result">תוצאה</label>
							<select id="result" class="form-group mb-3" name="result" required>
                                  <option value="טרם"<?php if ($result == 'טרם') echo ' selected'; ?>>טרם</option>
                                  <option value="עבר"<?php if ($result == 'עבר') echo ' selected'; ?>>עבר</option>
                                  <option value="לא עבר"<?php if ($result == 'לא עבר') echo ' selected'; ?>>לא עבר</option>
							</select>
					</div>
					<div class="col-sm-6"></div>
				</div>
...
<script src="scripts/validateTest.js"></script>
<footer class="mt-3" >
