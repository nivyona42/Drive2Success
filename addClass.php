...
<main>
    <?php
        ...
        $sql="insert into classes(idStudent,date, startTime, endTime, destination) values ('".$id."','".$date."','".$startTime."','". $endTime."','". $destination."')";
        if ($conn->query($sql)==FALSE){
            echo "<p class='echoDesign'>לא ניתן להוסיף שיעור נהיגה חדש, שגיאה:  .$conn->error </p>";
        }else{
            echo "<p class='echoDesign'>שיעור נהיגה חדש נוסף בהצלחה!</p>";
        }
    ?>
     <div class="center">
