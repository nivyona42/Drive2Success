...
<main>
    <?php
	   $host="localhost";
        ...
        $sql = "UPDATE cars SET id='$carID', company='$company', year='$year',model='$model', color='$color', gir='$gir', price='$price', testDate='$testDate', insuranceDate='$insuranceDate'  WHERE id='$prevId'";
  
        if ($conn->query($sql)==FALSE){
            echo "<p class='echoDesign'>לא ניתן לעדכן את פרטי כלי הרכב , שגיאה:  .$conn->error </p>";
        }else{
            echo "<p class='echoDesign'>פרטי כלי הרכב  עודכנו בהצלחה!</p>";
        }
    ?>
     <div class="center">
