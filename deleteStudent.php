...
<header class="container-fluid sticky-top">
  ...
  <span id="menuHeading" class="navbar-text ms-auto">&nbsp;מחיקת תלמיד </span>
...
<main>
    <?php
        ...
        $sql = "DROP TABLE goals_$id";
        if ($conn->query($sql) === FALSE) {
            echo "<p class='echoDesign'>לא ניתן למחוק את המטרות האישיות של התלמיד, שגיאה: " . $conn->error . "</p>";
        } else {
            $sql = "DELETE FROM students WHERE id=$id";

        if ($conn->query($sql)==FALSE){
            echo "<p class='echoDesign'>לא ניתן למחוק את התלמיד, שגיאה:  .$conn->error </p>";
        }else{
            echo "<p class='echoDesign'>התלמיד נמחק בהצלחה!</p>";
        }
        }
