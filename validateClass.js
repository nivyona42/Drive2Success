function validateForm() {
    ...
    if (!endTime) {
     ...
     let destination = validatDest(document.forms["myForm"]["destination"].value, document.forms["myForm"]["destination"].value);
    if (!destination) {
        document.forms["myForm"]["destination"].focus();
        return false;
    }
    return true;
}
...
function validatDest(dest){
		    var hebrewEnglishAndNumbersRegex = /^[\u0590-\u05FFa-zA-Z0-9\s]+$/;
		    if (!dest.value.match(hebrewEnglishAndNumbersRegex)) {
		          	alert("הכתובת אינה תקינה .הכנס בבקשה רק אותיות או מספרים.");
		            return false;
  			}
  			return true;
		}
