...
<header class="container-fluid sticky-top">
  ...
  <span id="menuHeading" class="navbar-text ms-auto">&nbsp; מחיקת שיעור נהיגה </span>
...
<main>
    <?php
     ...
     $sql = "DELETE FROM classes WHERE idStudent=$id AND date='$date' AND startTime='$startTime'";
  
        if ($conn->query($sql)==FALSE){
            echo "<p class='echoDesign'>לא ניתן למחוק את שיעור הנהיגה, שגיאה:  .$conn->error </p>";
        }else{
            echo "<p class='echoDesign'>שיעור הנהיגה נמחק בהצלחה!</p>";
        }
    mysqli_close($conn);
    ?>
     <div class="center">
