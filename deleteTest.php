...
<header class="container-fluid sticky-top">
  ...
  <span id="menuHeading" class="navbar-text ms-auto">&nbsp; מחיקת מבחן </span>
...
<main>
    <?php
     ...
     $sql = "DELETE FROM tests WHERE idStudent=$id AND date='$date'";

        if ($conn->query($sql)==FALSE){
            echo "<p class='echoDesign'>לא ניתן למחוק את המבחן, שגיאה:  .$conn->error </p>";
        }else{
            echo "<p class='echoDesign'>המבחן נמחק בהצלחה!</p>";
        }
    mysqli_close($conn);
    ?>
     <div class="center">
